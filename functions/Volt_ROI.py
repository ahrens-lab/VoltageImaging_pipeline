...
def get_spikes(trace, superfactor=10, threshs=(.4, .6, .75)):
    ...
    for iters in range(3):
        ...
        high_freq_med = np.array(pd.Series(high_freq).rolling(window=9000,min_periods=4500,center=True).median())
        high_freq_std = np.array(pd.Series(high_freq).rolling(window=9000,min_periods=4500,center=True).std())
...
def optimize_trace(img,mean_img,cell_inds):
    ...
    norm_tcourse1=first_timecourse/np.array(pd.Series(first_timecourse).rolling(window=150,min_periods=75,center=True).quantile(0.8))
    ...
    if (isinstance(spiketimes1,int) or (not_optimize==1)):
        ...
        norm_tcourse2=second_timecourse/np.array(pd.Series(second_timecourse).rolling(window=150,min_periods=75,center=True).quantile(0.8))
