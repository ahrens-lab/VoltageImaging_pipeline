...
pathname=r"D:\Takashi\SPIM\06082017";
fname="Fish1-4"
...
model = load_model(r'C:\Users\kawashimat\Documents\Datasets\20x_voltron\06072017Fish1\cell_model_32.hdf5')
...
#gauss_image=gaussian_filter(rank_image,cell_radius*0.2)
minima_image=im.local_minima_dll(rank_image,int(cell_radius*1.5))
overlay=np.tile(im.imNormalize95(ave)[:,:,None],(1,1,3))/2
...
for i in range(len(cell_r)):
    imlist[i,0,:,:]=im.imNormalize99(ave_pad[cell_r[i]:(cell_r[i]+image_radius*2+1), \
          cell_c[i]:(cell_c[i]+image_radius*2+1)])[None,None,:,:]
...
cell_image=np.tile(im.imNormalize999(ave)[:,:,None],(1, 1, 3))
...
cell_inds1=ROI.proof_images(imlist,inds1)
...
fig=plt.figure(3)
plt.subplot(121).imshow(im.imNormalize999(ave_pad))
a2=plt.subplot(122)
xs=[]
ys=[]
while True:
...
plt.subplot(131).imshow(im.imNormalize999(ave.astype('float')), cmap='gray')
img_color=np.tile(im.imNormalize999(ave.astype('float'))[:,:,None],(1,1,3))
for n in range(nROI):
    inds=ROI_list[n]
